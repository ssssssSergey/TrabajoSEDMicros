...
static lv_disp_drv_t * driver_pendiente = nullptr;
...
//Callback de cuando el DMA termina (El "Timbre" de aviso)
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) {
    if (hspi == &hspi1) { //Verificamos que sea nuestro SPI

        //Subir el CS para cerrar la comunicación con la pantalla
        HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_SET);

        //Avisar a LVGL
        if (driver_pendiente != nullptr) {
            lv_disp_flush_ready(driver_pendiente);
            driver_pendiente = nullptr;
        }
    }
}
...
static void MX_SPI1_Init(void)
{
  ...
  hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_8;//estaba en 2
...
{
    //llama al gestor de interrupciones de HAL
